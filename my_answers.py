...
class NeuralNetwork(object):
    ...
    def backpropagation(self, final_outputs, hidden_outputs, X, y, delta_weights_i_h, delta_weights_h_o):
        ...
        #output_error_term = (-1) * error * self.weights_hidden_to_output * hidden_outputs
        ...
        delta_i_h = (-1) * error * final_outputs * np.matmul(np.vstack(X), np.vstack(1-hidden_outputs).T)
        delta_h_o = (-1) * error * hidden_outputs
        
        # Weight step (input to hidden)
        delta_weights_i_h += delta_i_h
        # Weight step (hidden to output)
        delta_weights_h_o += np.vstack(delta_h_o)
